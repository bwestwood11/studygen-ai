...
const ChatMessages = ({ messages, isPending }: { messages: Message[], isPending:boolean }) => {
    ...
    <div>
      <div className="flex-1 overflow-auto p-4">
        {messages.map((message, index) => (
          <ChatMessage message={message} user={session.data?.user} key={index} />
        ))}
...
const Copy = ({content}:{content:string}) => {
  ...
  <Button
    variant="ghost"
    size="icon"
    className="w-4 h-4 hover:bg-transparent text-stone-400 hover:text-stone-900" onClick={handleCopyClick}
  >
