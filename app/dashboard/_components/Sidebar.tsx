...
const Sidebar = ({credits}:{credits:number | undefined}) => {
  ...
  return (
    <div className="space-y-4 py-4 flex flex-col text-background top-[var(--nav-height)] sticky justify-between overflow-auto h-fullscreen border-r px-2 md:block w-[200px] xl:w-[290px]">
        ...
        {routes.map((route) => (
          ...
          >
            <div className="flex items-center flex-1">
              <route.icon className={cn("w-6 h-6 mr-3 group-hover:text-background", route.color)} />
              {route.label}
            </div>
          </Link>
        ))}
      ...
      <div className="space-y-1">
        ...
        {chatbotCustomizationRoutes.map((route) => (
            <Link
              href={route.href}
              key={route.href}
              className={cn("text-sm group flex p-4 cursor-pointer hover:text-background hover:bg-foreground/80 rounded-lg transition",
                `${
                  pathname === route.href
                    ? "bg-white/10 text-foreground"
                    : "text-foreground"
...
const Loader = () => {
  return (
    <div className="flex flex-row items-center gap-3 text-foreground h-[100px] mb-10 w-full p-4">
        {/* <UserAvatar/> */}
        <Avatar>
          <AvatarFallback className="animate-pulse rounded-md bg-muted">A</AvatarFallback>
        </Avatar>
